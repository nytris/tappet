...
class RegionAssertionRegistryTest extends AbstractTestCase
{
    ...
    public function testRegisterRegionAssertionHandlerMergesHandlersFromMultipleRegistrationsForSameRegionType(): void
    {
        $containsAssertion = new ExpectRegionContains('my-region', 'some text');
        $doesNotContainAssertion = new ExpectRegionDoesNotContain('my-region', 'other text');
        $containsHandlerCalled = false;
        $doesNotContainHandlerCalled = false;
        $containsHandler = mock(RegionAssertionHandlerInterface::class, [
            'getHandlers' => [
                ExpectRegionContains::class => function () use (&$containsHandlerCalled): void {
                    $containsHandlerCalled = true;
                },
            ],
        ]);
        $doesNotContainHandler = mock(RegionAssertionHandlerInterface::class, [
            'getHandlers' => [
                ExpectRegionDoesNotContain::class => function () use (&$doesNotContainHandlerCalled): void {
                    $doesNotContainHandlerCalled = true;
                },
            ],
        ]);
