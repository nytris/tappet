...
namespace Tappet\Tests\Unit\Runner\Automation\Interaction;
...
class InteractionRegistryTest extends AbstractTestCase
{
    ...
    /** @var InteractionRegistry<AutomationInterface> */
    ...
    public function testRegisterInteractionHandlerMergesHandlersFromMultipleRegistrationsForSameInteractionType(): void
    {
        $enactInteraction = new Enact('my-button');
        $hoverInteraction = new Hover('my-button');
        $enactHandlerCalled = false;
        $hoverHandlerCalled = false;
        $enactHandler = mock(InteractionHandlerInterface::class, [
            'getHandlers' => [
                Enact::class => function () use (&$enactHandlerCalled): void {
                    $enactHandlerCalled = true;
                },
            ],
        ]);
        $hoverHandler = mock(InteractionHandlerInterface::class, [
            'getHandlers' => [
                Hover::class => function () use (&$hoverHandlerCalled): void {
                    $hoverHandlerCalled = true;
                },
            ],
        ]);
        $this->registry->registerInteractionHandler('button', $enactHandler);
        $this->registry->registerInteractionHandler('button', $hoverHandler);

        $this->registry->handleInteraction('button', $enactInteraction, $this->automation);
        $this->registry->handleInteraction('button', $hoverInteraction, $this->automation);

        static::assertTrue($enactHandlerCalled);
        static::assertTrue($hoverHandlerCalled);
    }
}
